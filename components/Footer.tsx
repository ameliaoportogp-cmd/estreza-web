...
export default function Footer() {
  return (
    <footer style={{ background: "linear-gradient(180deg, var(--navy-deep) 0%, #081628 100%)", color: "rgba(255,255,255,0.82)", fontFamily: "var(--font-body)", borderTop: "1px solid rgba(30,122,136,0.2)" }}>
      ...
      <div className="container" style={{ padding: "68px 24px 30px" }}>
        <div className="responsive-grid-2" style={{ gridTemplateColumns: "1.4fr 1fr 1fr 1.2fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: "1.02rem", lineHeight: 1.82, color: "rgba(255,255,255,0.78)", marginBottom: 24, maxWidth: 320 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {sociales.map(s => (
                <a key={s.icon} href={s.href} target="_blank" rel="noopener noreferrer"
                  style={{ width: 36, height: 36, borderRadius: 8, border: "1px solid rgba(255,255,255,0.14)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.8rem", fontWeight: 700, color: "rgba(255,255,255,0.72)", transition: "var(--transition)" }}
                  onMouseOver={e => { const el = e.currentTarget; el.style.background = "var(--teal)"; el.style.color = "#fff"; el.style.borderColor = "var(--teal)"; }}
                  onMouseOut={e => { const el = e.currentTarget; el.style.background = "transparent"; el.style.color = "rgba(255,255,255,0.72)"; el.style.borderColor = "rgba(255,255,255,0.14)"; }}>
          ...
          {/* ── Servicios ── */}
          <div>
            <h4 style={{ color: "var(--teal-light)", fontWeight: 700, fontSize: "0.88rem", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 18 }}>Servicios</h4>
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 10 }}>
              {links.servicios.map(([label, href]) => (
                <li key={href}>
                  <Link href={href}
                    style={{ fontSize: "1rem", color: "rgba(255,255,255,0.76)", transition: "var(--transition)", display: "flex", alignItems: "center", gap: 6 }}
                    onMouseOver={e => (e.currentTarget.style.color = "var(--gold)")}
                    onMouseOut={e => (e.currentTarget.style.color = "rgba(255,255,255,0.76)")}>
          ...
          {/* ── Academia ── */}
          <div>
            <h4 style={{ color: "var(--teal-light)", fontWeight: 700, fontSize: "0.88rem", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 18 }}>Academia</h4>
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {/* ── Contacto ── */}
          <div>
            <h4 style={{ color: "var(--teal-light)", fontWeight: 700, fontSize: "0.88rem", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 18 }}>Contacto</h4>
            <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 24 }}>
              {[
                ...
                <div key={text} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  ...
                  <span style={{ fontSize: "1rem", color: "rgba(255,255,255,0.76)", lineHeight: 1.65 }}>{text}</span>
                </div>
              ))}
            </div>
            <Link href="/prueba-gratis" className="btn-primary" style={{ fontSize: "0.98rem", padding: "12px 22px" }}>
              Prueba Gratis →
            </Link>
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 22, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <p style={{ fontSize: "0.96rem", color: "rgba(255,255,255,0.58)" }}>
          ...
          <div style={{ display: "flex", gap: 20 }}>
            {[["Privacidad", "/privacidad"], ["Términos", "/terminos"]].map(([label, href]) => (
              <Link key={href} href={href}
                style={{ fontSize: "0.96rem", color: "rgba(255,255,255,0.58)", transition: "var(--transition)" }}
