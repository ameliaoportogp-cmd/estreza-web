...
const servicios = [
  { icon: "compliance" as const, titulo: "Cumplimiento Normativo", accentColor: "var(--gold)", href: "/servicios/cumplimiento",
    ...
    idealPara: "Ideal para instituciones con visitas próximas, observaciones abiertas o brechas documentales.",
  ...
  { icon: "audit" as const, titulo: "Auditoría de Sistemas TI", accentColor: "var(--teal)", href: "/servicios/auditoria-ti",
    ...
    idealPara: "Ideal para equipos de tecnología, auditoría interna y cumplimiento que necesitan evidencia técnica sólida.",
  ...
  { icon: "automation" as const, titulo: "Automatización de Procesos", accentColor: "var(--gold)", href: "/servicios/automatizacion",
    ...
    idealPara: "Ideal para áreas con procesos repetitivos, cuellos de botella y alto esfuerzo manual.",
  ...
  { icon: "operations" as const, titulo: "Procesos Operativos", accentColor: "var(--teal)", href: "/servicios/procesos",
    ...
    idealPara: "Ideal para instituciones que necesitan ordenar procesos, responsabilidades y métricas operativas.",
  ...
  { icon: "risk" as const, titulo: "Gestión de Riesgos", accentColor: "var(--gold)", href: "/servicios/riesgos",
    ...
    idealPara: "Ideal para fortalecer SARO, matrices, reportes y trazabilidad ante Alta Dirección y supervisor.",
    ...
    <>
      <section style={{ background: "linear-gradient(135deg, var(--navy-deep) 0%, var(--navy-light) 100%)", padding: "72px 0 64px", color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div className="container" style={{ position: "relative", zIndex: 1 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.82)", fontSize: "1.08rem", maxWidth: 620, lineHeight: 1.8 }}>
            ...
            {servicios.map((s) => (
              <div key={s.href} className="card service-feature-grid" style={{ borderLeft: `4px solid ${s.accentColor}`, padding: "36px 36px" }}>
                <div>
                  ...
                  <p style={{ color: "var(--gray-600)", lineHeight: 1.8, marginBottom: 16, fontSize: "1rem" }}>{s.desc}</p>
                  <p style={{ color: "var(--navy)", lineHeight: 1.7, marginBottom: 24, fontSize: "0.98rem", fontWeight: 600 }}>
                    {s.idealPara}
                  </p>
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                    <Link href={s.href} className="btn-primary">Ver detalle del servicio</Link>
                    <Link href="/contacto" className="btn-outline">Hablar con un especialista</Link>
                  </div>
                </div>
                <div style={{ background: "var(--gray-50)", borderRadius: 12, padding: "24px 28px" }}>
                  ...
                  <h4 style={{ fontWeight: 700, color: s.accentColor, marginBottom: 16, fontSize: "0.88rem", textTransform: "uppercase", letterSpacing: "0.1em" }}>Incluye</h4>
                  <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 12 }}>
                    {s.beneficios.map(b => (
                      <li key={b} style={{ display: "flex", gap: 12, alignItems: "flex-start", fontSize: "0.98rem", color: "var(--gray-600)", lineHeight: 1.65 }}>
      ...
      <section style={{ background: "linear-gradient(135deg, var(--teal) 0%, #155F6A 100%)", padding: "64px 0" }}>
        <div className="container" style={{ textAlign: "center" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.84)", marginBottom: 32, fontSize: "1.04rem" }}>Agenda una consulta gratuita y te orientamos según tu nivel de urgencia, brechas y prioridades regulatorias.</p>
